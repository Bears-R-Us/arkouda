...
(ul(0x428a2f98), ul(0x71374491), ul(0xb5c0fbcf), ul(0xe9b5dba5), ul(0x3956c25b),
 ...
 ul(0x90befffa), ul(0xa4506ceb), ul(0xbef9a3f7), ul(0xc67178f2));
  ...
  inline proc packBlock(buf: [?D] uint(8)): 16*uint(32) {
    var res: 16*uint(32);
    for (inByteInd, b) in zip(D, buf) {
      var outWord: int = ((inByteInd - D.low) / 4) + 1;
      var lshift: int = 8 * (3 - ((inByteInd - D.low) % 4));
      res[outWord] |= (b:uint(32)) << lshift;
    }
    return res;
  }

  proc SHA256Digest(msg: [?D] uint(8)): 8*uint(32) {
    const lastBlockBytes: int = (D.size % 64);
    const fullBlocks: int = (D.size / 64);
    var sha = new SHA256State();
    for b in 0..#fullBlocks {
      var block = packBlock(msg[D.low+(64*b)..#64]);
      sha.fullblock(block);
    }
    var block = packBlock(msg[D.low+(64*fullBlocks)..#lastBlockBytes]);
    return sha.lastblock(block, (8*lastBlockBytes):uint);
  }
