...
module BinOp
{
  ...
  */
  ...
  proc doBigIntBinOpvv(l, r, op: string) throws {
    ...
    if l.etype == bigint && r.etype != bigint && (new set(string, ["<<", ">>", "**"])).contains(op) {
      ...
      select op {
        ...
        when "-" { forall (t, ri) in zip(tmp, ra) do t -= ri: bigint; }
        when "*" { forall (t, ri) in zip(tmp, ra) do t *= ri: bigint; }
        when "//" { forall (t, ri) in zip(tmp, ra) do t = if ri != 0 then t / ri: bigint else 0: bigint; }
        when "%" {
  ...
  proc doBigIntBinOpvvBoolReturnRealInput(l, r, op: string) throws {

    ref la = l.a;
    ref ra = r.a;
    var e = makeDistArray((...l.tupShape), bool);
    ref ea = e;
