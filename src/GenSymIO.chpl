...
//      }
      ...
      if (mode == APPEND) {
        ...
        for loc in 0..#A.targetLocales().size {
          ...
          if file_id < 0 { // Negative file_id means error
            ...
            /*
             * If this is a Strings values dataset, loop through all locales and set the slice 
             * indices, which are used to remove uint(8) characters from the locale slice that are
             * part of a string that belongs to the previous locale in the pdarray list of locales.
             */
            if isStringsValuesDataset(dsetName) {
			...
			if isStringsSegmentsDataset(dsetName) {
              ...
              coforall (loc, idx) in zip(A.targetLocales(), filenames.domain) with (ref indices,
                ...
                if idx < A.targetLocales().size {
                  ...
                  var fileId = C_HDF5.H5Fopen(fileName.c_str(), C_HDF5.H5F_ACC_RDWR,
                                                                       C_HDF5.H5P_DEFAULT);
                  const locDom = A.localSubdomain();
                  var indexValues = getLowerAndUpperStringsValueBounds(fileId, group, 
                                                                      A.localSlice(locDom));
                  localeValuesFileIndexStart[idx] = indexValues[0];
                  localeValuesFileIndexEnd[idx] = indexValues[1];

                  /*
                   * Close the file now that the values indices have been retrieved and the
                   * segments slices have been assigned
                   */
              ...
              coforall (loc, idx) in zip(A.targetLocales(), filenames.domain) with(ref indices,
                ...
                writeln("FOR locale %t THE UPDATED START: %t AND END: %t".format(idx, start, end));
            ...
            coforall (loc, idx) in zip(A.targetLocales(), filenames.domain) with 
                    (ref indices, ref localeValuesIndexStart, ref localeValuesIndexEnd) do on loc {
              ...
              if isStringsValuesDataset(dsetName) {
                /*
                 * If mode == APPEND, the Strings dataset is going to be appended 
                ...
                if A.localSlice(locDom).back() != NULL_STRINGS_VALUE {
                  ...
                  on Locales[idx+1] {
                    const locDom = A.localSubdomain();
                               ...
                               writeln("THE INDEX BOUNDS for %t: %t".format(idx, indexBounds));

                               /*
								 * Write the valuesList containing the uint(8) characters missing from
								 * the current locale slice along with retrieved from the next locale to hdf5
								 */
								H5LTmake_dataset_WAR(myFileID, '/%s/values'.format(group).c_str(), 1,
										c_ptrTo(dims), getHDF5Type(A.eltType), c_ptrTo(valuesList.toArray()));
								H5LTmake_dataset_WAR(myFileID, '/%s/values-index-bounds'.format(group).c_str(), 1,
										c_ptrTo([indexBounds.size]), getHDF5Type(int),
										c_ptrTo(indexBounds));
								...
								if sliceIndex == -1 {
									...
									//                   c_ptrTo([segmentsList.size:uint(64)]),getHDF5Type(int), 
									//                   c_ptrTo(segmentsList.toArray()));  
									var indexBounds = getLocaleValuesIndexBounds(localeValuesIndexStart, localeValuesIndexEnd,
											idx, A.localSlice(locDom).size);
									writeln("THE INDEX BOUNDS for %t: %t".format(idx, indexBounds));
									H5LTmake_dataset_WAR(myFileID, '/%s/values'.format(group).c_str(), 1,
											c_ptrTo(dims), getHDF5Type(A.eltType),
											c_ptrTo(A.localSlice(locDom)));
                                    H5LTmake_dataset_WAR(myFileID, '/%s/values-index-bounds'.format(group).c_str(), 1,
											c_ptrTo([[indexBounds.size:uint(64)]]), getHDF5Type(int),
											c_ptrTo(indexBounds));
								} else {
									/*
									 ...
									 * the segments list and then finally write the values and segments arrays 
									 * out to hdf5
									 */
									var charList = convertLocalSliceToList(A, locDom);
							...
							writeln("Locale %t SEGMENTS BEFORE ADJUSTMENT FOR START INDEX %t".format(idx, segments));
							var newSegmentsList = adjustForStringSegmentsSlices(localeValuesIndexStart[idx], segments);
							writeln("Locale %t SEGMENTS ADJUSTED FOR START INDEX %t".format(idx, newSegmentsList));
							if idx < filenames.size-1 {
								on Locales[idx+1] {
									const locDom = A.localSubdomain();
									var stopIndex = localeValuesIndexStart[idx+1];
									writeln("THE STOP INDEX for locale %t is %t".format(idx+1, stopIndex));
									for (value, i) in zip(A.localSlice(locDom),
											0..A.localSlice(locDom).size-1) {
										if value < stopIndex {
											newSegmentsList.append(value:int);
											writeln("UPDATED locale %t SEGMENTS WITH APPENDS %t".format(idx, newSegmentsList));
										} else {
											break;
										}
									}
							...
							writeln("NEW SEGMENTS ADJUSTED FOR APPEND %t".format(newSegmentsList));
							...
							/*
							 * This is neither a Strings values nor a Strings segments pdarray, so simply 
    ...
    /*
     * Converts a local slice into a int list for use in methods that add
     * or remove entries from the resulting list.
     */
    ...
    /*
     * Adjusts the segments list by removing leading segments at the position
     * specified by the startIndex parameter.
     */
    private proc adjustForStringSegmentsSlices(startIndex : int, 
                                                   segmentsList : list(int)) {
      try! writeln("THE START INDEX %t for segments %t".format(startIndex, segmentsList));
      var newSegmentsList: list(int, parSafe=true);
      for segment in segmentsList {
        try! writeln("TESTING SEGMENT %t AGAINST START INDEX %t".format(segment, startIndex));
        if segment >= startIndex {
          //segmentsList.remove(segment);
          try! writeln("APPENDING %t".format(segment));
    ...
    /*
     * Reads the Strings values lower and upper bounds for a file, which correspond to 
     * the values array chunk written to the file.
     */
    private proc getLowerAndUpperStringsValueBounds(fileId, group, segments) throws {
	...
	*/
     ...
     * or segments dataset corresponding to a Strings save operation.
     */
     ...
     */
	private proc rebaseSegmentsDataset(segments) {
      var dec : int;
	 ...
	 */
	private proc getLocaleValuesIndexBounds(indexStart, indexEnd, idx, numLocaleValues) {
		var start: int = 0;
		if (idx != 0) {
