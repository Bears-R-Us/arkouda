module Repartition
{
    ...
    ([PrivateSpace] list(int), [PrivateSpace] list(uint(8)))
  {
    ...
    stopCommDiagnostics();
    writeln("=== Comm for first coforall in repartitionByLocaleString ===");
    ...
    coforall loc in Locales do on loc {
      ...
      for i in 0..#numLocales {
        recvOffsets[i][here.id] = sendOffsets[i];
        recvBytes[i][here.id] = sendBytes[i];
      }

    }

    stopCommDiagnostics();
    writeln("=== Comm for 2nd coforall in repartitionByLocaleString ===");
    printCommDiagnosticsTable();
    resetCommDiagnostics();
    ...
    coforall loc in Locales do on loc {
      ...
      var numBytesReceivedByLoc = numBytesReceivingByLocale[here.id];
      var numStringsReceivedByLoc = numStringsReceivingByLocale[here.id];
      var myRecvOffsets = recvOffsets[here.id];
      var myRecvBytes = recvBytes[here.id];
      ...
      var pieces: [0..4] int = 0;

      for i in 0..#numLocales {

        const byteOffsetThisLoc = byteOffsetAdjuster[i];
        const idxOffsetThisLoc = idxOffsetAdjuster[i];

        for j in 0..#numStringsReceivedByLoc[i] {
          ...
          var startInd = myRecvOffsets[i][j];
          if here.id == 1 {
                stopCommDiagnostics();
                pieces[0] += getCommDiagnosticsHere().get: int;
                resetCommDiagnostics();
          }
          if here.id == 1 {
            startCommDiagnostics();
          }
          startInd += byteOffsetThisLoc;
          if here.id == 1 {
                stopCommDiagnostics();
                pieces[1] += getCommDiagnosticsHere().get: int;
                resetCommDiagnostics();
          }
          if here.id == 1 {
            startCommDiagnostics();
          }
          var startInd2 = myRecvOffsets[i][j];
          if here.id == 1 {
                stopCommDiagnostics();
                pieces[2] += getCommDiagnosticsHere().get: int;
                resetCommDiagnostics();
          }
          if here.id == 1 {
            startCommDiagnostics();
          }
          var tempBytes = myRecvBytes[i][startInd2..#size];
          if here.id == 1 {
                stopCommDiagnostics();
                pieces[3] += getCommDiagnosticsHere().get: int;
                resetCommDiagnostics();
          }
          if here.id == 1 {
            startCommDiagnostics();
          }
          myBytes[startInd..#size]
            = tempBytes;
          if here.id == 1 {
                stopCommDiagnostics();
                pieces[4] += getCommDiagnosticsHere().get: int;
                resetCommDiagnostics();
          }
        }
      ...
      if here.id == 1 {
        writeln("Comms for each piece: ", pieces);
      }
