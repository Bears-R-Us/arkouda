...
The initial use case for Arkouda external integration is Kubernetes as described below. Since the ExternalSystem delegates registration logic to Curl, building Arkouda with ExternalSystem requires the libcurl4-openssl-dev lib to be installed. For Debian Linux distros, the install command is as follows:
...
Note that the arkouda_server.externalSystem param is SystemType.KUBERNETES (Kubernetes is the external system Arkouda is registering with) and the ExternalSystem.serviceType param is ServiceType.INTERNAL (Arkouda is deployed within of Kubernetes)

#### Kubernetes External Service Registration (Slurm)

An example Slurm BATCH file for an Arkouda instance that registers/deregisters with Kubernetes is shown below. Note that the arkouda_server.externalSystem param is SystemType.KUBERNETES (Kubernetes is the external system Arkouda is registering with) and the ExternalSystem.serviceType param is ServiceType.EXTERNAL (Arkouda is deployed outside of Kubernetes)
...
export KEY_FILE=/opt/arkouda/tls.key #on slurm hosts
export CERT_FILE=/opt/arkouda/tls.crt #on slurm hosts
export CACERT_FILE=/etc/kubernetes/ssl/kube-ca.pem #on slurm hosts

./arkouda_server -nl 3 --arkouda_server.externalSystem=SystemType.KUBERNETES \
                       --ExternalSystem.serviceType=ServiceType.EXTERNAL \
                       --logLevel=LogLevel.DEBUG
```

#### Kubernetes External Service Registration (Bare Metal)

An example bare metal deployment script for an Arkouda instance that registers/deregisters with Kubernetes is shown below. As is the case with the Arkouda-on-Slurm deployment, the arkouda_server.externalSystem param is SystemType.KUBERNETES and the ExternalSystem.serviceType param is ServiceType.EXTERNAL
