...
class SegArray:
    def __init__(self, segments, values, copy=False, lengths=None, grouping=None):
        ...
        if not ak.is_sorted(segments) or (ak.unique(segments).size != segments.size):
        ...
        if grouping is None:
            if self.size == 0:
                ...
                self.grouping = ak.GroupBy(ak.broadcast(self.segments, ak.arange(self.size), self.valsize))
        else:
    ...
    def concat(cls, x, axis=0, ordered=True):
        ...
        if axis == 0:
            ...
            nonzero = ak.concatenate((newsegs[:-1] < newsegs[1:], ak.array([True])))
    ...
    def _get_lengths(self):
        if self.size == 0:
            ...
            return ak.concatenate((self.segments[1:], ak.array([self.valsize]))) - self.segments
    
    def __getitem__(self, i):
        if ak.isSupportedInt(i):
            start = self.segments[i]
            end = self.segments[i] + self.lengths[i]
            return self.values[start:end].to_ndarray()
        elif (isinstance(i, ak.pdarray) and (i.dtype == ak.int64 or i.dtype == ak.bool)) or isinstance(i, slice):
    ...
    def get_ngrams(self, n, return_origins=True):
        ...
        if return_origins:
            origin_indices = self.grouping.broadcast(ak.arange(self.size), permute=True)[:valid.size][valid]
            return ngrams, origin_indices
        else:
            return ngrams
    
    def _normalize_index(self, j):
        if not ak.isSupportedInt(j):
    ...
    def append_single(self, x, prepend=False):
        ...
        origscatter = ak.arange(self.valsize) + self.grouping.broadcast(ak.arange(self.size), permute=True)
        if prepend:
            origscatter += 1
    ...
    def save(self, prefix_path, dataset='segarray', segment_suffix='_segments', value_suffix='_values', mode='truncate'):
        '''
    ...
    def load(cls, prefix_path, dataset='segarray', segment_suffix='_segments', value_suffix='_values', mode='truncate'):
        ...
        if segment_suffix == value_suffix:
            raise ValueError("Segment suffix and value suffix must be different")
        segments = ak.load(prefix_path, dataset=dataset+segment_suffix)
        values = ak.load(prefix_path, dataset=dataset+value_suffix)
    ...
    def attach(cls, name, segment_suffix='_segments', value_suffix='_values', length_suffix='_lengths', grouping_suffix='_grouping'):
        if len(set((segment_suffix, value_suffix, length_suffix, grouping_suffix))) != 4:
            raise ValueError("Suffixes must all be different")
        return cls(ak.attach(name+segment_suffix), ak.attach(name+value_suffix), lengths=ak.attach(name+length_suffix), grouping=ak.attach(name+grouping_suffix))
