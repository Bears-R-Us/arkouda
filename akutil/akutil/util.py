...
def get_callback(x):
    if type(x) in {ak.Datetime, ak.Timedelta, aku.IPv4}:
        return type(x)
    elif hasattr(x, '_cast'):
        return x._cast
    elif type(x) == aku.BitVector:
        return aku.BitVectorizer(width=x.width, reverse=x.reverse)
    else:
        return identity
...
def register(a, name):
    ...
    if pd.to_datetime(ak.__version__.lstrip('v')) >= pd.to_datetime('2021.04.14'):
        ...
        cb = get_callback(a)
    if type(a) in {ak.Datetime, ak.Timedelta, aku.BitVector, aku.IPv4}:
        ...
        if type(a) in {ak.Datetime, ak.Timedelta}:
            reg._data.name = n
        else:
            reg.values.name = n
    else:
...
def attach_all(prefix):
    ...
    res = pat.findall(ak.information(ak.AllSymbols))
    return {k[len(prefix):]:ak.attach_pdarray(k) for k in res}
...
# def compute_features(data, keynames, aggregations, inplace=True):
#     try:
#         keys = data[keynames]
#         singleton = True
...
#     g = ak.GroupBy(keys)
#     if inplace:
#         for result, (func, col) in aggregations.items():
#             data[result] = g.broadcast(g.aggregate(data[col], func)[1], permute=True)
#     else:
#         return {result: g.broadcast(g.aggregate(data[col], func)[1], permute=True) for result, (func, col) in aggregations.items()}
...
def expand(size, segs, vals):
    ...
    """
    return ak.broadcast(segs, vals, size=size)
...
def arkouda_to_numpy(A: ak.pdarray, tmp_dir: str='') -> np.ndarray:
...
def convert_if_categorical(values):
    ...
    if isinstance(values, ak.Categorical):
