...
def time_ak_write(
    ...
    print("numLocales = {}, numNodes {}, N = {:,}, filesPerLoc = {}".format(cfg["numLocales"], cfg["numNodes"], N, numfiles))
    if dtype == "int64":
...
def check_correctness(dtype, path, seed, fileFormat, multifile=False):
    ...
    if dtype == "float64":
        assert np.allclose(
            a.to_ndarray(), c[0 : a.size].to_ndarray()
        )  # Slice is full array when single file
        if multifile:
