...
def time_ak_bfs_graph(trials:int):
    ...
    for _ in range(trials):
        start = time.time()
        level,nodes = ak.graph_bfs(Graph,0)
        end = time.time()
        timings.append(end - start)
    tavg = sum(timings) / trials
    print("Average time = {:.4f} sec".format(tavg))
    print("Average Edges = {:.4f} M/s".format(int(Graph.n_edges)/tavg/1024/1024))
    print("Average Vertices = {:.4f} M/s".format(int(Graph.n_vertices)/tavg/1024/1024))
    '''
