...
class Series:
    ...
    def __init__(self, ar_tuple=None,data=None, index=None):
        if ar_tuple is not None:
            ...
            if not isinstance(data,ak.pdarrayclass.pdarray) and not isinstance(data,ak.Categorical):
                data = ak.array(data)
            self.values= data
    ...
    def isin(self,lst):
        ...
        boolean = ak.in1d(self.values, lst)
    ...
    def value_counts(self,sort=True):
        ...
        dtype = aku.get_callback(self.values)
    ...
    def diff(self):
        ...
        values = ak.zeros(len(self), "float64") 
        values[1:] = ak.cast( self.values[1:] - self.values[:-1] ,"float64")
        values[0] = np.nan
