...
from arkouda.numpy.dtypes import int64 as ak_int64
from arkouda.numpy.dtypes import numpy_scalars


# histogram helper
def _convToDtp(dtp: Union[ak_int64], minVal, maxVal) -> Tuple[numpy_scalars, numpy_scalars]:
    """
    ...
    def conv(dtp, val, margin) -> numpy_scalars:
    ...
    return conv(dtp, minVal, -0.3), conv(dtp, maxVal, 0.3)
...
def histogram(
    ...
    if range:
        ...
        pda_min, pda_max = pda.min(), pda.max()
        if isinstance(
            pda_min,
            get_args(numpy_scalars),
        ) and isinstance(
            pda_max,
            get_args(numpy_scalars),
        ):
            minVal, maxVal = pda_min, pda_max
        else:
            raise ValueError
...
def histogram2d(
    ...
    if range:
        # resolve_scalar_dtype(val: object)
        (tmp_xMin, tmp_xMax), (tmp_yMin, tmp_yMax) = range
        xMin = cast(to_numpy_dtype(tmp_xMin), tmp_xMin)
        xMax = tmp_xMax.astype(to_numpy_dtype(tmp_xMax))
