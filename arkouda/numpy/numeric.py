...
# numeric with any pdarray -> pdarray (broadcast)
@overload
def where(
    condition: bool_scalars,
    A: pdarray,
...
def where(
    ...
    * ``Strings`` and ``Categorical`` are treated as 1D; we validate broadcast
    ...
    def _is_numeric_like(obj: operand) -> bool:
    ...
    if is_supported_bool(condition):
        if out_shape is None:
            # scalar condition + scalar x/y => scalar result
            return x if condition else y

        # scalar condition + any array-like => broadcast condition pdarray
        condition = ones(out_shape, dtype="bool") if condition else zeros(out_shape, dtype="bool")
