...
def matmul(pdaLeft: pdarray, pdaRight: pdarray) -> pdarray:
    ...
    """
    from arkouda.client import generic_msg
    from arkouda.numpy.pdarrayclass import broadcast_to_shape, dot
    ...
    # Handle the multi-dim cases.  This involves finding a common shape for broadcast.
        ...
        try:
            tmp_preshape = np.broadcast_shapes(left_preshape, right_preshape)  # upgrade to ak after
            tmp_pdaLeftshape = list(tmp_preshape)  # PR 4978 is merged.
            ...
            # change the next two lines to broadcast_to after PR 4978 is merged
            new_pdaLeft = broadcast_to_shape(pdaLeft, new_pdaLeftshape)
            new_pdaRight = broadcast_to_shape(pdaRight, new_pdaRightshape)  # args are now ready
