...
#  _isnan_w_bool is useful for ak.minimum and ak.maximum


def _isnan_w_bool(x):
    from arkouda.numpy.pdarraycreation import full

    return full(x.shape, False, dtype=ak_bool) if x.dtype == ak_bool else isnan(x)
...
def minimum(x1: Union[pdarray, numeric_scalars], x2: Union[pdarray, numeric_scalars]) -> pdarray:
    ...
    if np.isscalar(tx1) and isinstance(tx2, pdarray):
        return (
            full(nan, tx2.size)
            if np.isnan(tx1)
            else where(_isnan_w_bool(tx2), tx2, where(tx1 < tx2, tx1, tx2))
        )
    ...
    # This is because in numpy, there are different types of nans.
        ...
        if np.isnan(tx2):
            return where(_isnan_w_bool(tx1), tx1, tx2)  # nan special case
        else:
            return where(_isnan_w_bool(tx1), tx1, where(tx1 < tx2, tx1, tx2))
    ...
    #    tx2 where tx2 is a nan, otherwise the min of (tx1,tx2).
        ...
        return where(_isnan_w_bool(tx1), tx1, where(_isnan_w_bool(tx2), tx2, where(tx1 < tx2, tx1, tx2)))
