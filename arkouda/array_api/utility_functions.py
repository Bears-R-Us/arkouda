...
def trapz(y: Array, x: Optional[Array] = None, dx: Optional[float] = 1.0, axis: int = -1) -> Array:
    ...
    >>> y = x**2
    >>> xp.trapz(y, x) # doctest: +SKIP
    Arkouda Array ((), float64)0.3334027488546439

    Or estimate the area of a circle, noting we repeat the sample which closes
    the curve:

    >>> theta = xp.linspace(0, 2 * xp.pi, num=1000, endpoint=True)
    >>> xp.trapz(xp.cos(theta), x=xp.sin(theta)) # doctest: +SKIP
    Arkouda Array ((), float64)3.1415719413758416
