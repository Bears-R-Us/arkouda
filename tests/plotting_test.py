...
class TestPlotting:
    ...
    # --- NumPy-only tests (no Arkouda server required) ---------------------------
    ...
    # --- Arkouda-backed tests (skip if server is not connected) ------------------
    ...
    # --- Minimal smoke test for hist_all returning (fig, axes) -------------------

    # def test_hist_all_returns_fig_axes_numpy_dataframe(self,monkeypatch):
    #     """
    #     This test uses a very small, numpy-backed DataFrame-like
    #     to avoid requiring a live Arkouda server.
    #     If your hist_all requires a real ak.DataFrame,
    #     move this test under an Arkouda-connected skip like above.
    #     """
    #     class DummyAKDF:
