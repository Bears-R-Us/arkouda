...
def _segarray_to_lists(seg: ak.SegArray) -> list[list]:
    # SegArray.tolist() returns python nested lists already
    return seg.tolist()
...
def test_segarray_roundtrip_tolist(dtype):
    ...
    assert _segarray_to_lists(seg) == py
...
def test_segarray_getitem_boolmask(dtype):
    ...
    got = _segarray_to_lists(seg[akmask])
...
def test_segarray_concat_axis0(dtype):
    ...
    got = _segarray_to_lists(ak.SegArray.concat(segs, axis=0))
...
def test_segarray_concat_axis1(dtype):
    ...
    got = _segarray_to_lists(ak.SegArray.concat(segs, axis=1))
...
def test_segarray_append_matches_concat(dtype):
    ...
    got0 = _segarray_to_lists(sa.append(sb, axis=0))
    ...
    got1 = _segarray_to_lists(sa.append(sb, axis=1))
...
def test_segarray_append_single_and_prepend_single(dtype):
    ...
    got_app = _segarray_to_lists(seg.append_single(akx))
    exp_app = [row + [x[i]] for i, row in enumerate(py)]
    assert got_app == exp_app

    got_pre = _segarray_to_lists(seg.prepend_single(akx))
...
def test_segarray_unique(dtype):
    ...
    got = _segarray_to_lists(seg.unique())
