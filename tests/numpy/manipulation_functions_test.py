...
class TestNumpyManipulationFunctions:
    ...
    def test_repeat_pdarray(self, size, dtype):
        ...
        assert np.array_equal(np_f, f.to_ndarray())
    ...
    def test_repeat_dim_2(self, dtype, shape: Tuple[int, ...]):
        ...
        for i in shape:
            shape_prod *= i
        if dtype == ak.bigint:
            a = ak.arange(2 ** 200, 2 ** 200 + shape_prod, dtype=dtype).reshape(shape)
            np_a = np.arange(2 ** 200, 2 ** 200 + shape_prod, dtype=dtype).reshape(shape)
        else:
            a = ak.arange(shape_prod, dtype=dtype).reshape(shape)
            np_a = np.arange(shape_prod, dtype=dtype).reshape(shape)
        reps = akrandint(0, 10, 1, seed=seed)
