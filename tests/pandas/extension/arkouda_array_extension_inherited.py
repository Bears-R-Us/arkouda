...
from pandas.core.arrays import base as ea_base
...
class TestArkoudaArrayFillMaskInplaceInherited:
    @pytest.mark.parametrize(
        "EA, make_data",
        [
            # Numeric ArkoudaArray
            (ArkoudaArray, lambda: ak.arange(5, dtype=ak.int64)),
            # Strings EA
    ...
    def test_fill_mask_inplace_is_inherited(self, EA, make_data):
        """
        All Arkouda EAs should inherit _fill_mask_inplace from
        pandas.core.arrays.base.ExtensionArray.
        """
        assert "_fill_mask_inplace" not in EA.__dict__
        assert EA._fill_mask_inplace is ea_base.ExtensionArray._fill_mask_inplace

    @pytest.mark.xfail(
        reason=("Fails because Depends on ArkoudaStringArray.__setitem__ implementation #5100")
    )
    @pytest.mark.parametrize(
        "EA, make_data, expected",
        [
            # numeric
            (ArkoudaArray, lambda: ak.arange(5), [0, 0, 2, 2, 4]),
            # strings
            (ArkoudaStringArray, lambda: ak.array(["a", "b", "c", "d", "e"]), ["a", "a", "c", "c", "e"]),
            # categorical
            (
                ArkoudaCategoricalArray,
                lambda: ak.Categorical(ak.array(["x", "y", "z", "y", "x"])),
    ...
    def test_fill_mask_inplace_pad_basic(self, EA, make_data, expected):
        """method='pad' forward-fills masked positions, following pandas behavior."""
        a = EA(make_data())
        mask = np.array([False, True, False, True, False], dtype=bool)

        a._fill_mask_inplace(method="pad", limit=None, mask=mask)
    ...
    @pytest.mark.parametrize(
        ...
        [
            # numeric
            (ArkoudaArray, lambda: ak.arange(5), [0, 2, 2, 4, 4]),
            # strings
            (ArkoudaStringArray, lambda: ak.array(["a", "b", "c", "d", "e"]), ["a", "c", "c", "e", "e"]),
            # categorical
            (
                ArkoudaCategoricalArray,
                lambda: ak.Categorical(ak.array(["x", "y", "z", "y", "x"])),
    ...
    def test_fill_mask_inplace_backfill_basic(self, EA, make_data, expected):
        """method='backfill' fills masked positions from the next non-masked value."""
        a = EA(make_data())
        mask = np.array([False, True, False, True, False], dtype=bool)

        a._fill_mask_inplace(method="backfill", limit=None, mask=mask)
    ...
    def test_fill_mask_inplace_noop_when_mask_all_false(self, EA, make_data):
        """If mask is all False, values should remain unchanged."""
        data = make_data()
        a = EA(data)
        original = a.tolist()  # before modification
        mask = np.zeros(len(a), dtype=bool)

        a._fill_mask_inplace(method="pad", limit=None, mask=mask)

        assert a.tolist() == original
    ...
    @pytest.mark.parametrize(
        ...
        [
            # numeric
            (ArkoudaArray, lambda: ak.arange(5), [0, 0, 2, 3, 4]),
            # strings
            (ArkoudaStringArray, lambda: ak.array(["a", "b", "c", "d", "e"]), ["a", "a", "c", "d", "e"]),
            # categorical
            (
                ArkoudaCategoricalArray,
                lambda: ak.Categorical(ak.array(["x", "y", "z", "y", "x"])),
    ...
    def test_fill_mask_inplace_respects_limit(self, EA, make_data, expected):
        """limit=1 ensures only the FIRST masked element in each masked run is filled."""
        a = EA(make_data())

        mask = np.array([False, True, True, True, False], dtype=bool)

        a._fill_mask_inplace(method="pad", limit=1, mask=mask)
...
class TestArkoudaArrayRank:
    ...
    def test_rank_with_nan_matches_pandas_series(self):
        """
        When the data contain NaN, rank() should ignore NaNs by default
        (skipna=True) and preserve their positions, matching pandas.
        """
        values = np.array([10.0, np.nan, 30.0, 20.0], dtype="float64")

        s_expected = pd.Series(values)
        expected = s_expected.rank()  # default skipna=True
        ...
        # Same NA pattern
        assert result.isna().tolist() == expected.isna().tolist()

        # Compare only non-NA ranks
        mask = ~expected.isna()
...
class TestArkoudaArraySearchSorted:
    ...
    # ----------------------------------------------------------------------
    # Categoricals: searchsorted currently broken via astype(object)
    # ----------------------------------------------------------------------

    def test_searchsorted_scalar_categorical_currently_raises_typeerror(self):
        """
        ArkoudaCategoricalArray currently cannot support searchsorted because
        ExtensionArray.searchsorted calls self.astype(object), and our
        ArkoudaCategoricalArray.astype implementation requires an explicit
        dtype argument.

        Lock in this TypeError so we notice when we fix astype/searchsorted.
        """
        arr = ArkoudaCategoricalArray(ak.Categorical(ak.array(["a", "c", "e"])))

        with pytest.raises(TypeError):
            arr.searchsorted("c", side="right")

        with pytest.raises(TypeError):
            arr.searchsorted("b", side="left")

    def test_searchsorted_array_like_categorical_currently_raises_typeerror(self):
        """
        Array-like searchsorted on ArkoudaCategoricalArray also currently
        fails for the same reason (astype(object) signature mismatch).
        """
        arr = ArkoudaCategoricalArray(ak.Categorical(ak.array(["a", "c", "e"])))
        values_arr = np.asarray(["a", "b", "e", "z"], dtype=object)

        with pytest.raises(TypeError):
            arr.searchsorted(values_arr, side="left")

    def test_searchsorted_empty_array_base_categorical_currently_raises_typeerror(self):
        """
        For an empty ArkoudaCategoricalArray, searchsorted currently fails
        in the same astype(object) path. We lock in the TypeError.
        """
        arr = ArkoudaCategoricalArray(ak.Categorical(ak.array([], dtype=ak.str_)))
