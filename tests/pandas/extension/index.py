...
class TestIndexExtension:
    def test_index_basic_ops(self):
        idx = Index(ArkoudaArray(ak.array([10, 20, 30])), name="foo")
        assert len(idx) == 3
        assert idx.name == "foo"
        assert idx[0] == 10
    ...
    def test_index_equality(self):
        a = Index(ArkoudaArray(ak.array([1, 2, 3])))
        b = Index(ArkoudaArray(ak.array([1, 2, 3])))
        c = Index(ArkoudaArray(ak.array([3, 2, 1])))
