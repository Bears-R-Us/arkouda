...
proc testBroadcast() {
  ...
  var rep_msg = broadcastMsg(cmd="broadcast", payload=reqMsg, st).msg;
  var resName = parseName(rep_msg);
  ...
  rep_msg = broadcastMsg(cmd="broadcast", payload=reqMsg, st).msg;
  var res2Name = parseName(rep_msg);
