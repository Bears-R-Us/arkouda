...
proc testMessageLayer(substr, n, minLen, maxLen) throws {
  ...
  writeReq(reqMsg);
  var rep_msg = segmentedPeelMsg(cmd="segmentedPeel", payload=reqMsg, st).msg;
  writeRep(rep_msg);
  var (loAttribs,lvAttribs,roAttribs,rvAttribs) = rep_msg.splitMsgToTuple('+', 4);
  var loname = parseName(loAttribs);
  var roname = parseName(roAttribs);
  reqMsg = "stick str %s str %s False %jt".format(loname, roname, [""]);
  writeReq(reqMsg);
  rep_msg = segBinopvvMsg(cmd="segBinopvv", payload=reqMsg, st).msg;
  writeRep(rep_msg);
  var rtoname = parseName(rep_msg);
