/*
This is the fixed-length string optimization Parquet read performance
benchmark.

What this means is that we can create the array before reading the
file just with knowing the number of rows in the column, making it
so we don't need to read in the bytes for each column.
 */
...
// number of bytes to allcoate for each string
config const fixed_len_size = 6;
...
proc main() {
  ...
  t2.start();
  var seg: [0..#len] int = fixed_len_size;
  var sizes: [0..#1] int = len;
  seg = (+ scan seg) - seg;
  var ret: [0..#len*fixed_len_size] uint(8);
  var byteSizes: [0..#1] int = ret.size;
  t2.stop();
